...
`evalhub-hexagon-v1` 固定执行 60 次调用：知识、指令遵循、数学、综合推理、代码各 10 条，
安全可信由 TruthfulQA 和 BBQ 各 5 条合并。它是用于本地能力画像的精选 Mini Suite，不是完整
...
每个资产均以官方 URL、固定 Git revision（MMLU 为固定归档 SHA-256）和文件 SHA-256 三元组
校验；完整清单的 SHA-256 也会与 source revisions、提示模板版本和 generation config 一起写入
最终结果的 `reproducibility`。来源分别是 MMLU、IFEval、GSM8K、BIG-Bench Hard、HumanEval、
TruthfulQA 和 BBQ；固定值及官方链接见 [README](../../README.md#professional-hexagon-mini-suite)。

模型适配器和评分器只使用英文题面及官方英文参考答案。`input_zh`、`reference_zh` 和中文维度名
均为非官方的显示翻译；它们不会发送给模型、评分器或 HumanEval Docker 容器。
...
非 root 用户、最小权限和临时 `/tmp`。Docker 或镜像不可用时代码节点会 `blocked` 并给出构建命令；
