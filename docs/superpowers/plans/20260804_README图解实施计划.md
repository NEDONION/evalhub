...
**Goal:** 为 README 增加 5 张面向新用户的 GitHub Mermaid 图，解释 EvalHub 的系统全景、当前架构、启动顺序、评测流程与 Agent Benchmark 演进路线。
...
- Keep: `docs/architecture/20260804_系统架构.md` 作为更完整的架构说明链接。

**Interfaces:**
- Consumes: 当前 README“下一步建议”与 `docs/architecture/20260804_系统架构.md` 的目标生产架构。
- Produces: 一条四阶段 Agent Benchmark 演进路线、图例和最终验证结果。
...
在路线图前说明当前 Benchmark 仍是一次 LLM 补全，并说明蓝色代表当前能力、绿色代表下一步、灰色虚线代表后续规划；图后链接 `docs/product/20260804_Agent评测路线图.md` 和 `docs/architecture/20260804_EvalScope与EvalHub的Agent评测设计差异.md`，分别提供阶段验收标准与 EvalScope 架构取舍。
...
for file_path in docs/getting-started/20260804_Ollama本地模型安装与验证.md docs/development/20260804_Codex对话沉淀工作流.md docs/architecture/20260804_系统架构.md docs/architecture/20260804_EvalScope与EvalHub的Agent评测设计差异.md; do test -e "$file_path"; done
git diff --check -- README.md
```
...
Expected: README 和 Agent 评测正式文档只包含本次设计更新；工作区已有其他文件仍保持原状态且未被暂存。
