...
| `agent_session_started` | Codex 会话建立 | `sample_id`、`source_type`、`thread_id` |
| `agent_turn_started` | Codex 开始处理当前样本 | `sample_id`、`source_type` |
...
任务资源区域与最终结果之间复用 `EvaluationNodeInspector`，对 `agent_benchmark` 节点显示
“Agent 实时过程”。Agent 任务在运行中和完成后都显示。
...
- 默认按数据库事件顺序展示，运行中事件追加在底部。
- 事件显示发生时间、actor、中文类型、消息和最有价值的白名单证据。
- 工具输出、修改文件和 Verifier 摘要使用可复制的等宽正文，不倾倒任意 payload。
- 运行态时间线使用 `role=log` 和礼貌播报，状态不只依赖颜色。
- 旧任务没有 Agent 节点时不显示空时间线。

前端继续使用当前任务轮询。Agent 任务活动时，父级每秒取得新的运行耗时；节点变化指纹包含该
耗时，因此选中节点详情会继续读取最新事件。任务终止后停止轮询。这一实现复用现有轮询，不新增
第二个并发定时器。
