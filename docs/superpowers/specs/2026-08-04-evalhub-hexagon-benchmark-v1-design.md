...
包含知识、指令遵循、数学、综合推理、代码、安全可信六个维度，每维 5 个英文样本，
共 30 个样本。完整运行一次即可生成可重复比较的六边形能力画像。
...
- 套件稳定 ID 为 `evalhub-hexagon-v1`，当前协议版本为 `1.1.0`。
- 每个维度恰好评测 5 个固定样本，完整运行共调用模型 30 次。
...
- 模型对比必须使用相同套件版本、完整 30 个样本和相同生成配置。

## 非目标

- 30 个样本构成 EvalHub 的固定 Mini Suite，不宣称等同于任何来源 Benchmark 的全量分数。
...
revision 下载并校验；运行阶段只读取已经规范化的 30 个样本。

没有采用每轮随机抽样，因为不同模型会收到不同题目，无法精准比较。没有采用直接提交
30 道英文题到仓库，因为这会绕过用户要求的官方下载链路，也不利于核验来源 revision。

## 数据源与能力映射

Suite 由七个来源切片组成；安全可信维度由 TruthfulQA 3 题和 BBQ 2 题共同覆盖：

| Benchmark ID | 能力 | 官方来源 | 样本数 | 核心指标 |
| --- | --- | --- | ---: | --- |
| `hexagon-mmlu` | 知识 | MMLU test | 5 | Accuracy |
| `hexagon-ifeval` | 指令遵循 | IFEval | 5 | Prompt-level strict accuracy |
| `hexagon-gsm8k` | 数学 | GSM8K test | 5 | Numeric exact match |
| `hexagon-bbh` | 综合推理 | BIG-Bench Hard | 5 | Exact match / Accuracy |
| `hexagon-humaneval` | 代码 | HumanEval | 5 | Pass@1 |
| `hexagon-truthfulqa` | 安全可信 | TruthfulQA | 3 | Binary multiple-choice accuracy |
| `hexagon-bbq` | 安全可信 | BBQ | 2 | Three-choice accuracy |
...
- MMLU 从 5 个不同学科各选 1 题。
- IFEval 覆盖 5 个不同的可验证指令类型。
- GSM8K 从 test split 按固定种子哈希选择 5 题。
- BBH 从 5 个不同任务各选 1 题。
- HumanEval 按固定任务 ID 选择 5 题。
- TruthfulQA 从 3 个不同类别各选 1 题。
- BBQ 从 2 个不同偏见类别各选 1 题，并保留官方上下文类型。
...
使用官方 test CSV，保留原始问题、四个选项和答案字母。5 个样本来自不同学科，使用
现有 `choice_letter` 评分器计算 Accuracy。

### IFEval

使用官方英文 prompt 与 instruction ID。所选 5 题只覆盖能够本地确定性验证的官方
...
从 5 个 BBH 任务各取一个样本，保留任务提示结构，并按对应任务的官方目标执行精确
...
类型和类别。两者按样本数分别占安全可信维度 60% 和 40% 权重，合计正好 5 题。
...
选择 evalhub-hexagon-v1
  -> 读取固定选择清单
  -> 下载或复用七个官方来源缓存
  -> 校验 revision、原文件摘要和 30 个 source key
  -> 生成只含所选样本的规范化本地缓存
...
- 知识、指令遵循、数学、综合推理、代码均由各自 5 题的平均分得到。
- 安全可信为 TruthfulQA 3 题与 BBQ 2 题合并后的 5 题正确率；切片权重为 60% 和 40%。
- 六维全部完成时，总分为六维算术平均；因为每维都是 5 题，也等于 30 题总体平均。
- 单维最小可见分差为 20 分，同分不额外制造小数排名。
...
1. 选择清单恰好包含 30 个唯一逻辑 ID，每维恰好 5 个来源样本。
2. 七个来源切片数量严格为 5、5、5、5、5、3、2。
...
9. Oracle/固定适配器完整执行产生 30 个样本、六维满分和成功 Suite 状态。
10. 错误回答只降低对应维度，缺失 Docker 或损坏缓存产生未评测而不是虚假零分。
11. API 列出 `evalhub-hexagon-v1` 的六维覆盖、30 个配置样本和真实准备状态。
...
- 完整运行向模型发送恰好 30 个英文样本，六个维度各有 5 个有效评分。
