...
- Provider 错误、Agent 最大步数、超时、工具错误和等待人工审批：样本 Runner 错误，保留 Trace 并
  继续隐藏校验。
- 为使无头编码评测能够完成，桥只对 MiniClaw 已通过 `WorkspaceGuard` 校验、且 EvalHub 再次确认
  位于当前临时样本工作区内的 `write_file` / `edit_file` 做一次性批准，并设置每个样本最多 32 次的
  恢复上限。命令、网络、记忆和工作区外操作不得自动批准。
- 其他操作需要审批且尚未产生正确工作区时，样本按最终工作区评分为失败。
