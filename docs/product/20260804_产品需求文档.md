...
该功能只让模型与评分器处理英文官方题面和参考答案；中文题面、参考答案及能力标签均为非官方、
展示专用翻译。HumanEval 仅在本地固定 Docker 镜像中评分，容器禁网、只读且没有宿主挂载。Docker
或镜像缺失时，代码节点必须阻塞并公开唯一构建命令；其他已完成节点可形成部分能力画像，但不完整
套件不得参与模型排行榜。该精选 Mini Suite 不替代任何完整上游 Benchmark，产品不得将其分数宣传
为官方 MMLU、GSM8K、HumanEval 等成绩。
