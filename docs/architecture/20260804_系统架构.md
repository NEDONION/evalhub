...
Agent 路径固定 Pi CLI 壳，只替换 Ollama 基模，避免把“模型能力”和“不同 Agent 框架实现”混在
一次比较中。Pi 以 JSONL 输出工具事件，整个进程树由 macOS Seatbelt 限制为只写样本工作区且
只连接本机 Ollama。内置 Coding Mini v2 提供 6 个可审计编码任务，简单、中等、困难各 2 个；
请求可以选择全部或单档，题集版本、分级理由和每档通过率随结果持久化。
