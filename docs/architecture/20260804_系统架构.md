...
flowchart TD
    ...
    AGENT --> PI["Pi CLI + Ollama / DeepSeek 基模"]
...
Agent 路径固定 Pi CLI 壳，只替换 Ollama 或 DeepSeek 基模，避免把“模型能力”和“不同 Agent
框架实现”混在一次比较中。Pi 以 JSONL 输出工具事件，整个进程树由 macOS Seatbelt 限制为只写
样本工作区，并且只连接本机 Ollama 或不含真实凭据的本机受控代理。内置 Coding Mini v3 提供 6 个
可审计编码任务，简单、中等、困难各 2 个；协议预检、题集版本、分级报告和过程指标随结果持久化。
...
- `evalhub.agent`：外部 Agent Runner 协议、Pi JSONL 事件标准化和 Seatbelt 运行边界。
