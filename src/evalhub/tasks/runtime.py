...
class PersistentWorkflowExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            harness_asset_preparer: 准备核心 lm-eval 与代码任务官方资产的函数。
    ...
    def _prepare_assets(self, node: EvaluationNode) -> dict[str, object]:
        ...
        for benchmark_id in node.input.get("benchmark_ids", []):
            ...
            uses_harness = (
                spec.executor != ExecutorKind.NATIVE and spec.id != "hexagon-humaneval"
            )
            readiness = (
                ExecutorReadiness(True, "ready", "Harness 资产准备成功")
                if uses_harness
                else self._readiness_checker(spec)
            )
            try:
                preparer = self._harness_asset_preparer if uses_harness else self._asset_preparer
                path = preparer(spec.id)
    ...
    def _run_benchmark(
        ...
        *,
        ...
        if spec.executor != ExecutorKind.NATIVE and spec.id != "hexagon-humaneval":
            # 核心 Harness 可能按子任务宏平均，不能用样本均值覆盖官方聚合口径。
