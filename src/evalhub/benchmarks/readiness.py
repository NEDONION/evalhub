...
from evalhub.benchmarks.harness import benchmark_readiness as harness_benchmark_readiness
...
def benchmark_readiness(
    ...
    *,
    ...
    Returns:
        原生执行器直接就绪；核心外部任务沿用官方 Harness 探测；Hexagon HumanEval
        需 Docker 服务和固定镜像均成功；其他执行器返回 ``executor_not_ready``。
    """
    if spec.executor == ExecutorKind.NATIVE:
        return ExecutorReadiness(True, "ready", "本地原生执行器已就绪")
    if spec.dataset_revision == "resolved-at-runtime:sha256":
        ready, reason = harness_benchmark_readiness(spec)
        return ExecutorReadiness(
            ready,
            "ready" if ready else "executor_not_ready",
            "官方 Harness 执行器已就绪" if ready else str(reason or "执行器尚未配置"),
        )
